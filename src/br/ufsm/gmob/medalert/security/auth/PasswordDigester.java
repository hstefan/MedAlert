...
public class PasswordDigester {
	...
	public static DigestedPassInfo digest(String password, byte[] salt, int rounds) {
		...
		try {
			...
			e.printStackTrace();
			Log.e(PasswordDigester.class.getName(), "Failed to find SHA-512 hash algorithm.");
			return null;
		}
	}
	
	public static DigestedPassInfo digest(String password) {
		try {
			SecureRandom random = SecureRandom.getInstance("SHA1PRNG");
			return digest(password, random.generateSeed(DEFAULT_SALT_BYTES_NUM), DEFAULT_ROUND_NUM);
		} catch (NoSuchAlgorithmException e) {
			e.printStackTrace();
			Log.e(PasswordDigester.class.getName(), "Failed to find SHA1PRNG secure random algorithm.");
			return null;
		}
	}
}
